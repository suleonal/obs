...
public class StudentControllerApi {
     ...
     * @throws RestClientException if an error occurs while attempting to invoke the API
     */
    public List<Student> getStudentByLectureIdUsingGET(Integer lectureId, String token) throws RestClientException {
        Object postBody = null;
        
        // verify the required parameter 'lectureId' is set
        if (lectureId == null) {
            throw new HttpClientErrorException(HttpStatus.BAD_REQUEST, "Missing the required parameter 'lectureId' when calling getStudentByLectureIdUsingGET");
        }
        
        // verify the required parameter 'token' is set
        if (token == null) {
            throw new HttpClientErrorException(HttpStatus.BAD_REQUEST, "Missing the required parameter 'token' when calling getStudentByLectureIdUsingGET");
        }
        
        // create path and map variables
        final Map<String, Object> uriVariables = new HashMap<String, Object>();
        uriVariables.put("lectureId", lectureId);
        String path = UriComponentsBuilder.fromPath("/student/lecture-students/{lectureId}").buildAndExpand(uriVariables).toUriString();
